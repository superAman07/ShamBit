...
export class MediaService {
  ...
  private validateFile(uploadDto: UploadSignedUrlDto): void {
    ...
    const isImage =
      uploadDto.type === MediaType.PRODUCT_IMAGE ||
    ...
    if (isImage && !this.ALLOWED_IMAGE_TYPES.includes(uploadDto.mimeType)) {
      throw new BadRequestException(
        `Invalid image type. Allowed types: ${this.ALLOWED_IMAGE_TYPES.join(', ')}`,
      );
    }

    if (
      uploadDto.type === MediaType.DOCUMENT &&
      !this.ALLOWED_DOCUMENT_TYPES.includes(uploadDto.mimeType)
    ) {
      throw new BadRequestException(
        `Invalid document type. Allowed types: ${this.ALLOWED_DOCUMENT_TYPES.join(', ')}`,
      );
    }
  }
