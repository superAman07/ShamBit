...
export class Attribute {
  ...
  get isUsable(): boolean {
    return [AttributeStatus.ACTIVE, AttributeStatus.DEPRECATED].includes(
      this.status,
    );
  }

  get requiresOptions(): boolean {
    return [AttributeDataType.ENUM, AttributeDataType.MULTI_ENUM].includes(
      this.dataType,
    );
  }

  get isNumeric(): boolean {
    return [AttributeDataType.NUMBER, AttributeDataType.DECIMAL].includes(
      this.dataType,
    );
  }
  ...
  get isDate(): boolean {
    return [AttributeDataType.DATE, AttributeDataType.DATETIME].includes(
      this.dataType,
    );
  }

  get isFile(): boolean {
    return [AttributeDataType.IMAGE, AttributeDataType.FILE].includes(
      this.dataType,
    );
  }
  ...
  private validateStringValue(value: any, errors: string[]): void {
    ...
    if (
      ...
      value.length < this.validation.minLength
    ) {
  ...
  private validateEnumValue(value: any, errors: string[]): void {
    ...
    const validValues = this.options
