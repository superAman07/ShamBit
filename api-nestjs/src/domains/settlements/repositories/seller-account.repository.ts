...
export class SellerAccountRepository implements ISellerAccountRepository {
  ...
  async create(data: Partial<SellerAccount>): Promise<SellerAccount> {
    ...
    if (existingAccount) {
      throw new Error(
        `Seller account already exists for user ${data.sellerId}`,
      );
    }
    ...
    const account = await this.prisma.sellerAccount.create({
      data: {
        ...
        kycDocuments: data.kycDocuments
          ? JSON.stringify(data.kycDocuments)
          : undefined,
        verificationDetails: data.verificationDetails
          ? JSON.stringify(data.verificationDetails)
          : undefined,
  ...
  async findByStatus(
    ...
    return this.findAll({ status }, pagination, undefined, includes);
  }
  ...
  async findByKycStatus(
    ...
    return this.findAll({ kycStatus }, pagination, undefined, includes);
  }
  ...
  async findAccountsRequiringKyc(
    ...
    return this.findAll({ kycStatus: ['PENDING', 'REJECTED'] }, pagination);
  }
  ...
  private buildPagination(pagination?: PaginationOptions): {
    ...
    return {
      skip: pagination.offset
        ? parseInt(String(pagination.offset), 10)
        : (page - 1) * limit,
      take: limit,
    };
  }
