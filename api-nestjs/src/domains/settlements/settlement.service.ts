...
export class SettlementService {
  ...
  async findAll(
    ...
    this.loggerService.log('SettlementService.findAll', {
      filters,
      pagination,
      userId,
    });
  ...
  async findBySettlementId(
    ...
    this.loggerService.log('SettlementService.findBySettlementId', {
      settlementId,
      userId,
    });

    const settlement = await this.settlementRepository.findBySettlementId(
      settlementId,
      includes,
    );
    if (!settlement) {
  ...
  async createSettlement(dto: CreateSettlementDto): Promise<Settlement> {
    ...
    const sellerAccount = await this.sellerAccountRepository.findBySellerId(
      dto.sellerId,
    );
    if (!sellerAccount) {
    ...
    SettlementValidators.validateSettlementEligibility(
      sellerAccount,
      wallet,
      dto.netAmount,
    );
  ...
  async createBulkSettlements(dto: BulkSettlementDto): Promise<string> {
    ...
    // Validate input
    SettlementValidators.validateDateRange(
  ...
  async processSettlement(dto: ProcessSettlementDto): Promise<Settlement> {
    ...
    try {
      ...
      this.eventEmitter.emit(
        ...
        new SettlementProcessingStartedEvent(
          settlement.settlementId,
          settlement.sellerId,
          settlement.netAmount,
          dto.processedBy,
  ...
  async reconcileSettlement(dto: ReconcileSettlementDto): Promise<Settlement> {
    ...
    // Update settlement
    const updatedSettlement = await this.settlementRepository.update(
      settlement.id,
      {
  ...
  async processScheduledSettlements(): Promise<void> {
    ...
    try {
      ...
      const pendingSettlements =
        await this.settlementRepository.findPendingSettlements();
      ...
      this.logger.log(
        `Found ${pendingSettlements.data.length} pending settlements`,
      );
      ...
      for (const settlement of pendingSettlements.data) {
        try {
          ...
          this.logger.error(
            `Failed to process settlement ${settlement.settlementId}:`,
            error,
          );
        }
      }
  ...
  async retryFailedSettlements(): Promise<void> {
    ...
    try {
      ...
      const retryableSettlements =
        await this.settlementRepository.findRetryableSettlements();
      ...
      this.logger.log(
        `Found ${retryableSettlements.data.length} retryable settlements`,
      );
      ...
      for (const settlement of retryableSettlements.data) {
        try {
          ...
          this.logger.error(
            `Failed to retry settlement ${settlement.settlementId}:`,
            error,
          );
        }
      }
  ...
  private async processPayoutAsync(settlement: Settlement): Promise<void> {
    try {
      // Get seller account
      const sellerAccount = await this.sellerAccountRepository.findById(
        settlement.sellerAccountId,
      );
      if (!sellerAccount) {
        throw new Error(
          `Seller account not found: ${settlement.sellerAccountId}`,
        );
      }
      ...
      this.logger.error(
        `Payout processing failed for settlement ${settlement.settlementId}:`,
        error,
      );
  ...
  private async processBulkSettlementsAsync(
    jobId: string,
    dto: BulkSettlementDto,
  ): Promise<void> {
    ...
    try {
      ...
      await this.settlementJobService.updateJobStatus(
        jobId,
      ...
      for (const batch of batches) {
        const batchPromises = batch.map(async (sellerId) => {
          try {
            ...
            this.logger.error(
              `Failed to create settlement for seller ${sellerId}:`,
              error,
            );
            failureCount++;
          }
        ...
        // Update job progress
        await this.settlementJobService.updateJobProgress(
          jobId,
      ...
      this.logger.error(
        `Bulk settlement processing failed for job ${jobId}:`,
        error,
      );
