...
export class SettlementValidationService {
  ...
  async validateSettlementCreation(
    ...
    try {
      ...
      if (overlappingSettlements.length > 0) {
        warnings.push(
          `Found ${overlappingSettlements.length} overlapping settlements`,
        );
      }
      ...
      if (activeSettlements.length > 0) {
        errors.push(
          'Seller has active settlements that must be completed first',
        );
      }

      // Validate hold period
      const holdPeriodValidation = await this.validateHoldPeriod(
        sellerId,
        periodEnd,
      );
      ...
      const eligibilityCheck = await this.checkSettlementEligibility(
        sellerId,
        netAmount,
      );
  ...
  async validateSettlementProcessing(
    ...
    try {
      ...
      if (!sellerAccount) {
        ...
        const account = new SellerAccount({
          ...
          razorpayFundAccountId:
            sellerAccount.razorpayFundAccountId || undefined,
          kycDocuments: (sellerAccount.kycDocuments as any) || undefined,
          verificationDetails:
            (sellerAccount.verificationDetails as any) || undefined,
      ...
      const recentFailures = await this.getRecentFailedSettlements(
        settlement.sellerId,
      );
  ...
  async checkSettlementEligibility(
    ...
    if (!wallet) {
      ...
      if (!walletStatus.canSettle) {
        reasons.push(
          `Insufficient balance. Required: ₹${amount}, Available: ₹${walletStatus.availableBalance}`,
        );
      ...
      if (amount < walletStatus.minimumRequired) {
        reasons.push(
          `Amount below minimum settlement threshold of ₹${walletStatus.minimumRequired}`,
        );
        requirements.push(
          `Minimum settlement amount is ₹${walletStatus.minimumRequired}`,
        );
      }
    }
  ...
  async validateSellerAccountSetup(
    sellerId: string,
  ): Promise<ValidationResult> {
    const errors: string[] = [];
    const warnings: string[] = [];
  ...
  async validateSettlementLimits(
    ...
    if (period === 'DAILY') {
      ...
      if (todayTotal + amount > limits.dailyLimit) {
        errors.push(
          `Daily settlement limit exceeded. Limit: ₹${limits.dailyLimit}, Current: ₹${todayTotal}`,
        );
      }
    }
    ...
    if (monthlyTotal + amount > limits.monthlyLimit) {
      errors.push(
        `Monthly settlement limit exceeded. Limit: ₹${limits.monthlyLimit}, Current: ₹${monthlyTotal}`,
      );
    }
  ...
  private async checkSuspiciousActivity(sellerId: string): Promise<string[]> {
    ...
    const amounts = recentSettlements.map((s) =>
      parseFloat(s.netAmount.toString()),
    );
    if (amounts.length > 1) {
      const avgAmount =
