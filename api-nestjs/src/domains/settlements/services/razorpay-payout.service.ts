...
export class RazorpayPayoutService {
  ...
  private initializeRazorpay(): void {
    ...
    if (!keyId || !keySecret) {
      if (nodeEnv === 'production') {
        ...
        this.logger.warn(
          'Razorpay credentials not configured - running in development mode without Razorpay',
        );
        return;
      }
    }
  ...
  async createPayout(request: CreatePayoutRequest): Promise<PayoutResult> {
    ...
    try {
      ...
      const payoutData = {
        ...
        narration:
          request.description || `Settlement payout - ${request.settlementId}`,
        notes: {
      ...
      if (!this.razorpay) {
        throw new Error(
          'Razorpay service not initialized - please configure RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET',
        );
      }
  ...
  async getPayoutStatus(payoutId: string): Promise<PayoutResult> {
    this.loggerService.log('RazorpayPayoutService.getPayoutStatus', {
      payoutId,
    });
  ...
  async createFundAccount(sellerAccount: SellerAccount): Promise<{
    ...
    try {
      ...
      const fundAccount =
        await this.razorpay.fundAccount.create(fundAccountData);
  ...
  async handlePayoutWebhook(
    payload: any,
    signature: string,
  ): Promise<PayoutStatusUpdate | null> {
  ...
  async getAccountBalance(): Promise<{
    ...
    try {
      ...
      this.logger.error('Failed to fetch account balance', {
        error: error.message,
      });
  ...
  async getPayoutLimits(): Promise<{
    ...
    try {
      ...
      this.logger.error('Failed to fetch payout limits', {
        error: error.message,
      });
  ...
  private validateSellerAccount(sellerAccount: SellerAccount): void {
    ...
    if (!sellerAccount.razorpayFundAccountId) {
      throw new Error(
        'Seller account does not have Razorpay fund account configured',
      );
    }
  }
  ...
  async getPayoutHistory(
    ...
    try {
      ...
      this.logger.error('Failed to fetch payout history', {
        error: error.message,
      });
  ...
  async getPayoutSummary(
    ...
    try {
      ...
      this.logger.error('Failed to generate payout summary', {
        error: error.message,
      });
