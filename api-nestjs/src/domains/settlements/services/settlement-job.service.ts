...
export class SettlementJobService {
  ...
  async updateJobStatus(
    ...
    this.loggerService.log('SettlementJobService.updateJobStatus', {
      jobId,
      status,
    });
  ...
  async failJob(
    ...
    this.loggerService.log('SettlementJobService.failJob', {
      jobId,
      errorMessage,
    });
  ...
  async cancelStaleJobs(staleAfterHours: number = 24): Promise<number> {
    this.logger.log(
      `Cancelling stale jobs older than ${staleAfterHours} hours`,
    );
  ...
  async getJobStatistics(
    ...
    const averageDuration =
      jobsWithDuration > 0 ? totalDuration / jobsWithDuration : 0;
  ...
  async getJobHealth(): Promise<{
    ...
    if (activeJobs > 50 || staleJobs > 5 || recentFailures > 10)
      systemLoad = 'HIGH';
