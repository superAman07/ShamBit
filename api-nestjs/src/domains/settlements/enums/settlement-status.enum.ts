...
export const SettlementStatusTransitions: Record<
  SettlementStatus,
  SettlementStatus[]
> = {
...
export const SettlementJobStatusTransitions: Record<
  SettlementJobStatus,
  SettlementJobStatus[]
> = {
...
export function isSettlementTerminal(status: SettlementStatus): boolean {
  return [SettlementStatus.COMPLETED, SettlementStatus.CANCELLED].includes(
    status,
  );
}

export function isJobTerminal(status: SettlementJobStatus): boolean {
  return [SettlementJobStatus.COMPLETED, SettlementJobStatus.FAILED].includes(
    status,
  );
}
