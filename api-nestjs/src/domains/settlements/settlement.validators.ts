...
export class SettlementValidators {
  ...
  static validateSettlementProcessing(settlement: Settlement): void {
    ...
    if (!settlement.isProcessable()) {
      errors.push(
        `Settlement cannot be processed in status: ${settlement.status}`,
      );
    }
  ...
  static validateSettlementAmounts(data: any): void {
    ...
    if (data.grossAmount !== undefined && data.netAmount !== undefined) {
      ...
      if (Math.abs(calculatedNet - data.netAmount) > 0.01) {
        errors.push(
          `Net amount mismatch: calculated ${calculatedNet}, provided ${data.netAmount}`,
        );
      }
    }
  ...
  static validateSettlementLocking(
    settlement: Settlement,
    userId: string,
  ): void {
    if (settlement.isLocked() && settlement.lockedBy !== userId) {
      throw new ConflictException(
        `Settlement is locked by another user: ${settlement.lockedBy}`,
      );
    }
  }
  ...
  static validateSellerAccountCreation(data: any): void {
    ...
    if (
      data.accountType &&
      !['SAVINGS', 'CURRENT'].includes(data.accountType)
    ) {
  ...
  static validateSellerAccountVerification(account: SellerAccount): void {
    ...
    if (account.kycStatus !== KycStatus.SUBMITTED) {
      errors.push(
        `Cannot verify account with KYC status: ${account.kycStatus}`,
      );
    }
    ...
    if (errors.length > 0) {
      throw new BadRequestException(
        `Account verification validation failed: ${errors.join(', ')}`,
      );
    }
  }
  ...
  static validateWalletTransaction(data: any): void {
    ...
    if (
      !data.type ||
      !Object.values(WalletTransactionType).includes(data.type)
    ) {
    ...
    if (
      !data.category ||
      !Object.values(WalletTransactionCategory).includes(data.category)
    ) {
    ...
    if (errors.length > 0) {
      throw new BadRequestException(
        `Wallet transaction validation failed: ${errors.join(', ')}`,
      );
    }
  }
  ...
  static validateWalletBalance(wallet: SellerWallet): void {
    try {
      ...
      throw new BadRequestException(
        `Wallet balance validation failed: ${error.message}`,
      );
    }
  }
  ...
  static validateSettlementSchedule(data: any): void {
    ...
    if (
      !data.frequency ||
      !Object.values(SettlementFrequency).includes(data.frequency)
    ) {
  ...
  static validateSettlementEligibility(
    ...
    if (amount < minimumAmount) {
      errors.push(
        `Settlement amount ${amount} is below minimum ${minimumAmount}`,
      );
    }
    ...
    if (!wallet.canSettle(amount)) {
      errors.push(
        `Insufficient wallet balance for settlement amount ${amount}`,
      );
    }

    if (errors.length > 0) {
      throw new BadRequestException(
        `Settlement eligibility validation failed: ${errors.join(', ')}`,
      );
    }
  }
  ...
  static validateConcurrentSettlement(existingSettlements: Settlement[]): void {
    const activeSettlements = existingSettlements.filter(
      (s) =>
