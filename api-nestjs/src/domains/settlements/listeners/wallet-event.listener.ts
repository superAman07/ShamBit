...
export class WalletEventListener {
  ...
  async handleWalletCredited(event: WalletCreditedEvent) {
    ...
    try {
      ...
      await this.updateWalletAnalytics(
        event.sellerId,
        'CREDIT',
        event.amount,
        event.category,
  ...
  private async processBalanceRules(
    sellerId: string,
    newBalance: number,
  ): Promise<void> {
    try {
      ...
      const milestone = milestones.find(
        (m) => newBalance >= m && newBalance - 1000 < m, // Crossed milestone recently
      );
