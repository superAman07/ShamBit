...
export class SellerAccountController {
  ...
  @ApiOperation({
    ...
    description:
      'Get public seller information for ecommerce listings. Returns only seller name, store name, and verification status.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Get complete seller account information including sensitive data. Admin access required.',
  })
  @ApiResponse({
    status: 200,
  ...
  async findAllAdmin(
    ...
    return this.sellerAccountRepository.findAll(
      filters,
      pagination,
      undefined,
      includes,
  ...
  @ApiResponse({
    status: 200,
    description: 'Seller account retrieved successfully',
  })
  ...
  async findBySellerId(
    ...
    try {
      ...
      const account = await this.sellerAccountRepository.findBySellerId(
        sellerId,
        includes,
      );
  ...
  @ApiOperation({
    ...
    description:
      'Create a new seller account with bank details and business information. Allows BUYER users to upgrade to SELLER role.',
  })
  @ApiBody({
  ...
  async createSellerAccount(
    ...
    try {
      ...
      if (error.code === 'P2002') {
        throw new BadRequestException(
          'A seller account with this information already exists',
        );
      }

      if (error.code === 'P2003') {
        throw new BadRequestException(
          'Invalid seller ID - user does not exist',
        );
      }
  ...
  @ApiOperation({
    ...
    description:
      'Submit KYC documents for verification. Required for account activation.',
  })
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid KYC data or account not eligible for KYC submission',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Razorpay integration setup successfully',
  })
  @ApiResponse({
    status: 400,
    description: 'Failed to setup Razorpay integration',
  })
  @UseGuards(AuthGuard, RolesGuard)
  @Roles(UserRole.ADMIN, UserRole.FINANCE)
  async setupRazorpayIntegration(
    ...
    const fundAccountResult =
      await this.razorpayPayoutService.createFundAccount(account);
  ...
  async validateAccountSetup(
    ...
    return this.settlementValidationService.validateSellerAccountSetup(
      account.sellerId,
    );
  }
