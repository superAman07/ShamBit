...
export class SettlementResponse {
  @ApiProperty({
    ...
    example: 'stl_123456789',
  })
  id: string;

  @ApiProperty({
    description: 'Human-readable settlement ID',
    example: 'STL-2024-001',
  })
  settlementId: string;
  ...
  @ApiProperty({ description: 'Commission amount', example: 500.0 })
  commissionAmount: number;

  @ApiPropertyOptional({ description: 'Platform fee amount', example: 100.0 })
  platformFeeAmount?: number;

  @ApiPropertyOptional({ description: 'Tax amount', example: 90.0 })
  taxAmount?: number;

  @ApiPropertyOptional({ description: 'Adjustment amount', example: -10.0 })
  adjustmentAmount?: number;

  @ApiProperty({ description: 'Net amount to be paid', example: 9310.0 })
  netAmount: number;
  ...
  @ApiPropertyOptional({
    description: 'Scheduled processing date',
...
export class WalletBalanceResponse {
  @ApiProperty({
    description: 'Available balance for settlement',
    example: 5000.0,
  })
  availableBalance: number;

  @ApiProperty({
    description: 'Pending balance (clearance pending)',
    example: 2000.0,
  })
  pendingBalance: number;

  @ApiProperty({
    description: 'Reserved balance (for refunds)',
    example: 500.0,
  })
  reservedBalance: number;

  @ApiProperty({ description: 'Total wallet balance', example: 7500.0 })
  totalBalance: number;
}
...
export class WalletResponse {
  ...
  @ApiPropertyOptional({
    description: 'Last settlement amount',
    example: 4500.0,
  })
...
export class WalletTransactionResponse {
  ...
  @ApiProperty({ description: 'Transaction amount', example: 1000.0 })
  amount: number;
  ...
  @ApiProperty({ description: 'Balance before transaction', example: 4000.0 })
  balanceBefore: number;

  @ApiProperty({ description: 'Balance after transaction', example: 5000.0 })
  balanceAfter: number;
...
export class SettlementCalculationResponse {
  ...
  @ApiProperty({
    description: 'Calculation period start',
    example: '2024-01-01T00:00:00Z',
  })
  periodStart: string;

  @ApiProperty({
    description: 'Calculation period end',
    example: '2024-01-31T23:59:59Z',
  })
  periodEnd: string;

  @ApiProperty({ description: 'Total gross amount', example: 10000.0 })
  grossAmount: number;

  @ApiProperty({ description: 'Total commission amount', example: 500.0 })
  commissionAmount: number;

  @ApiProperty({ description: 'Platform fee amount', example: 100.0 })
  platformFeeAmount: number;

  @ApiProperty({ description: 'Tax amount', example: 90.0 })
  taxAmount: number;

  @ApiProperty({ description: 'Net amount payable', example: 9310.0 })
  netAmount: number;
...
export class ValidationResponse {
  ...
  @ApiProperty({
    ...
    type: [String],
    example: ['Settlement amount is below minimum threshold'],
  })
  warnings: string[];
}
...
export class ErrorResponse {
  ...
  @ApiPropertyOptional({
    ...
    example: ['grossAmount must be a positive number'],
  })
  details?: string[];
