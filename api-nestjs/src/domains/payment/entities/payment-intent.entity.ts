...
export class PaymentIntent {
  ...
  removePaymentMethod(method: PaymentMethod): void {
    this.allowedPaymentMethods = this.allowedPaymentMethods.filter(
      (m) => m !== method,
    );
  }
  ...
  getTotalRefunded(): number {
    return (
      this.transactions
        ?.filter((t) => t.type === 'REFUND' && t.isSuccessful())
        .reduce((sum, t) => sum + t.amount, 0) || 0
    );
  }
  ...
  getOutstandingAmount(): number {
    return Math.max(
      0,
      this.amount - this.getTotalPaid() + this.getTotalRefunded(),
    );
  }
  ...
  validate(): { isValid: boolean; errors: string[] } {
    ...
    if (this.applicationFee !== undefined) {
      ...
      if (this.applicationFee >= this.amount)
        errors.push('Application fee cannot exceed payment amount');
    }
  ...
  static createForMultiSeller(data: {
    ...
    const applicationFee = Math.round(
      data.amount * (data.platformFeePercentage / 100),
    );
