...
export class StripeGateway implements IPaymentGateway {
  ...
  async createPaymentIntent(
    ...
    try {
      ...
      const createParams: Stripe.PaymentIntentCreateParams = {
        ...
        confirmation_method:
          (request.confirmationMethod?.toLowerCase() as any) || 'automatic',
        capture_method:
          (request.captureMethod?.toLowerCase() as any) || 'automatic',
      ...
      const response: CreatePaymentIntentResponse = {
        ...
        nextAction: paymentIntent.next_action
          ? {
              ...
              redirectUrl: (paymentIntent.next_action as any).redirect_to_url
                ?.url,
  ...
  async retrievePaymentIntent(intentId: string): Promise<GatewayResponse<any>> {
    ...
    try {
      ...
      this.logger.error('Failed to retrieve Stripe payment intent', error, {
        intentId,
      });
  ...
  async confirmPaymentIntent(
    ...
    try {
      ...
      if (request.paymentMethod) {
        if (
          request.paymentMethod.type === PaymentMethod.CARD &&
  ...
  async capturePayment(
    ...
    try {
      ...
      this.logger.error('Failed to capture Stripe payment', error, {
        intentId,
        amount,
      });
