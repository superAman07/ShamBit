...
export class SearchResponseUtil {
  ...
  static processFacets(aggregations: any): SearchFacets {
    const processBuckets = (buckets: any[], nameField?: string): FacetBucket[] => {
    ...
    const facets: SearchFacets = {
      categories: processBuckets(aggregations.categories?.buckets || [], 'category_name'),
    ...
    if (aggregations.attributes?.attribute_names?.buckets) {
      aggregations.attributes.attribute_names.buckets.forEach((attrBucket: any) => {
        const attributeName = attrBucket.key;
        const values = processBuckets(attrBucket.attribute_values?.buckets || []);
    ...
    if (aggregations.availability?.buckets) {
      const availabilityBuckets = aggregations.availability.buckets.map((bucket: any) => ({
  ...
  static calculateRelevanceScore(hit: any, query: any): number {
    ...
    if (query.q && hit._source.name?.toLowerCase().includes(query.q.toLowerCase())) {
      score *= 1.2;
    }
