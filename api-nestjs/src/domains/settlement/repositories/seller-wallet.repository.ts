...
export class SellerWalletRepository implements ISellerWalletRepository {
  ...
  async findById(id: string, includes?: WalletIncludeOptions): Promise<SellerWallet | null> {
  ...
  async findBySellerId(sellerId: string, includes?: WalletIncludeOptions): Promise<SellerWallet | null> {
    const wallet = await this.prisma.sellerWallet.findUnique({
      where: { sellerId },
      include: this.buildInclude(includes),
    });
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const transaction = await (tx as any).sellerWalletTransaction.create({
        data: {
          ...
          referenceType: transactionData.referenceType,
          referenceId: transactionData.referenceId,
          description: transactionData.description || `Move pending to available`,
          metadata: transactionData.metadata || {},
          processedAt: new Date(),
          createdAt: new Date(),
        },
  ...
  private buildWhereClause(filters?: WalletFilters): any {
    ...
    if (filters.lastSettlementFrom || filters.lastSettlementTo) {
      ...
      if (filters.lastSettlementFrom) where.lastSettlementAt.gte = filters.lastSettlementFrom;
      if (filters.lastSettlementTo) where.lastSettlementAt.lte = filters.lastSettlementTo;
    }
  ...
  private mapToEntity(wallet: any): SellerWallet {
    return new SellerWallet({
      ...
      transactions: wallet.transactions?.map((t: any) => this.mapTransactionToEntity(t)),
    });
  }
