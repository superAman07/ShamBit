...
export class SettlementRepository implements ISettlementRepository {
  ...
  async lockForProcessing(id: string, userId: string): Promise<Settlement> {
    try {
      ...
      if (error.code === 'P2025') { // Record not found
        throw new ConflictException('Settlement is already locked or not found');
      }
      throw error;
    }
  }
  ...
  private buildWhereClause(filters?: SettlementFilters): any {
    ...
    if (filters.netAmountMin !== undefined || filters.netAmountMax !== undefined) {
      where.netAmount = {};
      if (filters.netAmountMin !== undefined) where.netAmount.gte = filters.netAmountMin;
      if (filters.netAmountMax !== undefined) where.netAmount.lte = filters.netAmountMax;
    }
