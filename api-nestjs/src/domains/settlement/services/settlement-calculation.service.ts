...
export class SettlementCalculationService {
  ...
  async calculateSettlement(
    ...
    const orders = await this.getCompletedOrders(sellerId, periodStart, periodEnd);

    if (orders.length === 0) {
      return this.createEmptyResult(sellerId, sellerAccount.id, periodStart, periodEnd, currency);
    }
    ...
    for (const order of orders) {
      for (const item of order.items) {
        const itemCalculation = await this.calculateOrderItemSettlement(item, order);
    ...
    const adjustmentAmount = await this.calculateAdjustments(sellerId, periodStart, periodEnd);

    // Calculate net amount
    const netAmount = totalGrossAmount - totalCommissionAmount - totalPlatformFeeAmount - totalTaxAmount + adjustmentAmount;
  ...
  async calculateOrderItemSettlement(orderItem: any, order: any): Promise<{
    grossAmount: number;
    commissionRate: number;
    commissionAmount: number;
    platformFeeAmount: number;
    ...
    let commissionRate = parseFloat(orderItem.commissionRate?.toString() || '0');
    let commissionAmount = parseFloat(orderItem.commissionAmount?.toString() || '0');

    // If no stored commission, calculate dynamically
    if (commissionRate === 0 || commissionAmount === 0) {
      const calculatedCommission = await this.calculateCommission(orderItem, order);
    ...
    // Calculate net amount
    const netAmount = grossAmount - commissionAmount - platformFeeAmount - taxAmount;
  ...
  async calculateCommission(orderItem: any, order: any): Promise<{
    rate: number;
    amount: number;
    ruleId?: string;
  }> {
    ...
    switch (rule.type) {
      ...
      case 'TIERED':
        const tieredResult = this.calculateTieredCommission(grossAmount, rule.tiers);
  ...
  private calculateTieredCommission(amount: number, tiers: any): { rate: number; amount: number } {
  ...
  async calculateTax(orderItem: any, order: any): Promise<number> {
    ...
    const commissionAmount = parseFloat(orderItem.commissionAmount?.toString() || '0');
    ...
    for (const refund of refunds) {
      for (const item of refund.items) {
        if (item.variant.product.sellerId === sellerId) {
          ...
          const commissionAmount = parseFloat(item.commissionAmount?.toString() || '0');
  ...
  private async getApplicableCommissionRules(
    ...
    return rules.map(rule => ({
      ...
      fixedAmount: rule.fixedAmount ? parseFloat(rule.fixedAmount.toString()) : undefined,
      minAmount: rule.minAmount ? parseFloat(rule.minAmount.toString()) : undefined,
      maxAmount: rule.maxAmount ? parseFloat(rule.maxAmount.toString()) : undefined,
    })) as CommissionRule[];
  }
  ...
  async validateSettlementPeriod(
    ...
    if (existingSettlements.length > 0) {
      warnings.push(`Found ${existingSettlements.length} existing settlements overlapping with this period`);
    }
    ...
    if (now < holdPeriodEnd) {
      ...
      warnings.push(`Settlement is in hold period. ${remainingDays} days remaining.`);
    }
  ...
  async getSettlementSummary(
    ...
    const calculation = await this.calculateSettlement(sellerId, periodStart, periodEnd);
    ...
    return {
      ...
      averageOrderValue: calculation.orderCount > 0 ? calculation.grossAmount / calculation.orderCount : 0,
      averageCommissionRate: calculation.grossAmount > 0 ? calculation.commissionAmount / calculation.grossAmount : 0,
    };
  }
}