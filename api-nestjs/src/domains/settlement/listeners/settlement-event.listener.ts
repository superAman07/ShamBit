...
export class SettlementEventListener {
  ...
  async handleSettlementCreated(event: SettlementCreatedEvent) {
    ...
    try {
      ...
      const wallet = await this.sellerWalletRepository.findBySellerId(event.sellerId);
  ...
  async handleSettlementProcessingStarted(event: SettlementProcessingStartedEvent) {
  ...
  async handleSettlementCompleted(event: SettlementCompletedEvent) {
    ...
    try {
      ...
      await this.sendSettlementNotification(event.sellerId, 'SETTLEMENT_COMPLETED', {
        settlementId: event.settlementId,
        amount: event.netAmount,
        razorpayPayoutId: event.razorpayPayoutId,
      });
  ...
  async handleSettlementFailed(event: SettlementFailedEvent) {
    ...
    try {
      ...
      await this.sendSettlementNotification(event.sellerId, 'SETTLEMENT_FAILED', {
        settlementId: event.settlementId,
        amount: event.netAmount,
        failureReason: event.failureReason,
        retryCount: event.retryCount,
  ...
  async handleSettlementCancelled(event: SettlementCancelledEvent) {
    ...
    try {
      ...
      if (wallet) {
        ...
        this.logger.log('Reserved settlement amount released due to cancellation', {
          settlementId: event.settlementId,
          walletId: wallet.id,
          amount: event.netAmount,
        });
