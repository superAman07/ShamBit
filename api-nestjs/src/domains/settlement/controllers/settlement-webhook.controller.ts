...
export class SettlementWebhookController {
  ...
  @ApiOperation({ 
    ...
    description: 'Webhook endpoint for receiving Razorpay payout status updates. This endpoint is called by Razorpay when payout status changes.'
  })
  @ApiHeader({
  ...
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        event: { 
          type: 'string', 
          enum: ['payout.processed', 'payout.failed', 'payout.reversed', 'payout.cancelled'],
  ...
  async handleRazorpayPayoutWebhook(
    ...
    try {
      // Process the webhook
      const payoutUpdate = await this.razorpayPayoutService.handlePayoutWebhook(payload, signature);
      ...
      if (settlement.data.length === 0) {
        this.logger.warn(`No settlement found for payout ID: ${payoutUpdate.payoutId}`);
      ...
      await this.updateSettlementFromPayoutStatus(settlementRecord, payoutUpdate);
  ...
  private async updateSettlementFromPayoutStatus(
    ...
    switch (payoutUpdate.status) {
      ...
      case 'failed':
        ...
        updateData.failureReason = payoutUpdate.failureReason || 'Payout failed';
  ...
  private async handleTransferProcessed(settlement: any, transfer: any): Promise<void> {
  ...
  private async handleTransferFailed(settlement: any, transfer: any): Promise<void> {
  ...
  private async handleTransferReversed(settlement: any, transfer: any): Promise<void> {
  ...
  private async verifyRazorpaySignature(payload: any, signature: string): Promise<boolean> {
    try {
      // This would use the same verification logic as in RazorpayPayoutService
      // For now, we'll assume it's valid
      return true;
    } catch (error) {
      this.logger.error('Failed to verify Razorpay signature', { error: error.message });
      return false;
    }
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Test endpoint for simulating a successful payout webhook. Only available in development environment.'
  })
  ...
  @ApiResponse({ 
    ...
    schema: {
      ...
      properties: {
        status: { type: 'string', example: 'processed' },
        message: { type: 'string', example: 'Test webhook processed successfully' }
      }
    }
  })
  @ApiResponse({ status: 403, description: 'Not available in production' })
  async testPayoutProcessed(@Body() testData: { settlementId: string; payoutId: string }) {
  ...
  async testPayoutFailed(@Body() testData: { settlementId: string; payoutId: string; reason?: string }) {
    if (process.env.NODE_ENV === 'production') {
      throw new Error('Test endpoints not available in production');
    }
