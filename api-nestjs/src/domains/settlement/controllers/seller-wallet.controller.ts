...
export class SellerWalletController {
  ...
  @ApiResponse({ status: 200, description: 'Seller wallets retrieved successfully' })
  @Roles('ADMIN', 'FINANCE')
  async findAll(
    ...
    return this.sellerWalletRepository.findAll(filters, pagination, undefined, includes);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get seller wallet by ID' })
  @ApiResponse({ status: 200, description: 'Seller wallet retrieved successfully' })
  ...
  @ApiResponse({ status: 201, description: 'Seller wallet created successfully' })
  ...
  @ApiOperation({ 
    ...
    description: 'Add funds to a seller wallet. Used for order payments, refunds, and adjustments.'
  })
  ...
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        amount: { type: 'number', minimum: 0.01, example: 1000.00 },
        category: { 
          type: 'string', 
  ...
  async reserveBalance(
    ...
    return this.sellerWalletRepository.reserveBalance(
      id,
      reserveDto.amount,
      {
        transactionId: this.generateTransactionId(),
  ...
  @ApiResponse({ status: 200, description: 'Reserved balance released successfully' })
  ...
  async releaseReserve(
    ...
    return this.sellerWalletRepository.releaseReserve(
      id,
      releaseDto.amount,
      {
        transactionId: this.generateTransactionId(),
  ...
  async getWalletTransactions(
    ...
    return {
      message: 'Wallet transactions endpoint - to be implemented with WalletTransactionRepository',
      walletId: id,
      filters,
      pagination,
    };
  ...
  @ApiResponse({ status: 200, description: 'Wallets with balance retrieved successfully' })
  @Roles('ADMIN', 'FINANCE')
  async getWalletsWithBalance(
    @Query('minBalance') minBalance: number = 0,
    @Query() pagination: PaginationOptions = {},
  ) {
    return this.sellerWalletRepository.findWalletsWithBalance(minBalance, pagination);
  }

  @Get('for-settlement')
  @ApiOperation({ summary: 'Get wallets eligible for settlement' })
  @ApiResponse({ status: 200, description: 'Settlement-eligible wallets retrieved successfully' })
  @Roles('ADMIN', 'FINANCE')
  async getWalletsForSettlement(
    @Query('minAmount') minAmount: number = 100,
    @Query() pagination: PaginationOptions = {},
  ) {
    return this.sellerWalletRepository.findWalletsForSettlement(minAmount, pagination);
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Get detailed balance information for the authenticated seller'
  })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      ...
      properties: {
        ...
        balances: {
          type: 'object',
          properties: {
            availableBalance: { type: 'number', description: 'Available for settlement' },
            pendingBalance: { type: 'number', description: 'Pending clearance' },
            reservedBalance: { type: 'number', description: 'Reserved for refunds' },
            totalBalance: { type: 'number', description: 'Total wallet balance' }
          }
        },
        currency: { type: 'string', example: 'INR' },
        settlableAmount: { type: 'number', description: 'Amount available for settlement' },
        lastSettlement: {
          type: 'object',
          properties: {
  ...
  async getMyWalletTransactions(
    ...
    // This would be implemented in a WalletTransactionRepository
    return {
      message: 'My wallet transactions endpoint - to be implemented with WalletTransactionRepository',
      walletId: wallet.id,
      sellerId: user.id,
      filters,
      pagination,
  ...
  async initializeMyWallet(
    ...
    const existingWallet = await this.sellerWalletRepository.findBySellerId(user.id);
    if (existingWallet) {
