...
export class SellerWallet {
  ...
  updateTotalBalance(): void {
    this.totalBalance = this.availableBalance + this.pendingBalance + this.reservedBalance;
    this.updatedAt = new Date();
  }
  
  validateBalances(): void {
    const calculatedTotal = this.availableBalance + this.pendingBalance + this.reservedBalance;
  ...
  credit(amount: number, category: WalletTransactionCategory): WalletBalanceSnapshot {
    if (amount <= 0) {
  ...
  debit(amount: number, category: WalletTransactionCategory): WalletBalanceSnapshot {
    if (amount <= 0) {
