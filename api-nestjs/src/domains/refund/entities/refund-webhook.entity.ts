export class RefundWebhook {
  ...
  getProcessingTimeInSeconds(): number | null {
    ...
    return Math.floor(
      (this.processedAt.getTime() - this.receivedAt.getTime()) / 1000,
    );
  }

  getAgeInMinutes(): number {
    return Math.floor(
      (new Date().getTime() - this.receivedAt.getTime()) / (1000 * 60),
    );
  }
  ...
  getEventCategory(): string {
    ...
    if (
      this.eventType.includes('processed') ||
      this.eventType.includes('completed')
    ) {
      return 'SUCCESS';
    }
    ...
    if (
      this.eventType.includes('updated') ||
      this.eventType.includes('changed')
    ) {
      return 'UPDATE';
    }
  ...
  isHighPriority(): boolean {
    const highPriorityEvents = ['refund.processed', 'refund.failed'];
