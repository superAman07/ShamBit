...
export class Refund {
  ...
  isTerminal(): boolean {
    return [RefundStatus.COMPLETED, RefundStatus.CANCELLED].includes(
      this.status,
    );
  }
  ...
  canBeRetried(): boolean {
    return (
      this.status === RefundStatus.FAILED && this.retryCount < this.maxRetries
    );
  }
  ...
  getNetRefundAmount(): number {
    return Math.max(
      0,
      this.approvedAmount - this.refundFees - this.adjustmentAmount,
    );
  }
  ...
  getProcessingTimeInMinutes(): number | null {
    ...
    return Math.floor(
      (this.processedAt.getTime() - this.createdAt.getTime()) / (1000 * 60),
    );
  }
  ...
  getApprovalTimeInMinutes(): number | null {
    ...
    return Math.floor(
      (this.approvedAt.getTime() - this.createdAt.getTime()) / (1000 * 60),
    );
  }
...
export class RefundItem {
  ...
  getApprovalPercentage(): number {
    return this.requestedQuantity > 0
      ? (this.approvedQuantity / this.requestedQuantity) * 100
      : 0;
  }
}
