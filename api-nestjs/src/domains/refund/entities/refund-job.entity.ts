...
export class RefundJob {
  ...
  getWaitingTimeInMinutes(): number | null {
    if (!this.startedAt) {
      return Math.floor(
        (new Date().getTime() - this.createdAt.getTime()) / (1000 * 60),
      );
    }
    return Math.floor(
      (this.startedAt.getTime() - this.createdAt.getTime()) / (1000 * 60),
    );
  }
    ...
    switch (this.type) {
      ...
      case RefundJobType.SEND_NOTIFICATION:
        if (!this.payload?.notificationType) {
          errors.push(
            'Notification type is required for send notification job',
          );
        }
        break;
