...
export class CategoryReparentingAlgorithm {
  ...
  private async validateReparenting(
    ...
    if (newParentId) {
      ...
      if (!newParent) {
        ...
        const childCountForParent = await this.prisma.category.count({
    ...
    if (childCount > 100) {
      warnings.push(
        `Category has ${childCount} children - operation may take longer`,
      );
    }
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        ...
        if (options.updateProducts) {
          ...
          const productCountForCategory = await tx.product.count({
  ...
  > {
    ...
    return operations
      .map((op) => ({
        ...op,
        depth: (depthMap.get(op.categoryId) as number) || 0,
      }))
