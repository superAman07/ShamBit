...
export class CategoryTreeQueries {
  ...
  async getCategoryTree(rootId?: string, maxDepth: number = 3) {
    ...
    if (rootId) {
      where.OR = [{ id: rootId }, { path: { contains: rootId } }];
  ...
  async getCategoriesWithProductCounts(parentId?: string) {
    ...
    if (parentId) {
      where.OR = [{ id: parentId }, { path: { contains: parentId } }];
    }
  ...
  async getFeaturedCategoriesTree() {
    ...
    const allPathIds = featuredCategories.flatMap((cat) =>
      (cat.path || '').split('/').filter(Boolean),
    );
  ...
  async bulkUpdatePaths(
    updates: Array<{ id: string; newPath: string; newDepth: number }>,
  ) {
