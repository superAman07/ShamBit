...
export class CategoryValidators {
  ...
  static validateCategoryDescription(description?: string): void {
    if (description) {
      if (description.length > 2000) {
        throw new BadRequestException(
          'Category description must not exceed 2000 characters',
        );
      }
    }
  }

  static validateSeoFields(
    seoTitle?: string,
    ...
    if (seoTitle) {
      ...
      if (!titleValidation.isValid) {
        throw new BadRequestException(
          `SEO Title: ${titleValidation.errors.join('; ')}`,
        );
      }
    }
    ...
    if (seoDescription) {
      const descValidation =
        CategoryPolicies.validateSeoDescription(seoDescription);
      if (!descValidation.isValid) {
        throw new BadRequestException(
          `SEO Description: ${descValidation.errors.join('; ')}`,
        );
      }
    }
    ...
    if (seoKeywords) {
      if (seoKeywords.length > 10) {
        throw new BadRequestException(
          'SEO keywords should not exceed 10 items',
        );
      }
      ...
      for (const keyword of seoKeywords) {
        if (keyword.length > 50) {
          throw new BadRequestException(
            'Each SEO keyword should not exceed 50 characters',
          );
        }
      }
    }
  ...
  static validateBrandIds(brandIds: string[], fieldName: string): void {
    ...
    for (const brandId of brandIds) {
      if (typeof brandId !== 'string' || !cuidPattern.test(brandId)) {
        throw new BadRequestException(
          `Invalid brand ID format in ${fieldName}: ${brandId}`,
        );
      }
    }
    ...
    if (uniqueIds.size !== brandIds.length) {
      throw new BadRequestException(
        `Duplicate brand IDs are not allowed in ${fieldName}`,
      );
    }
  }
  ...
  static validateTreeDepth(depth: number): void {
    ...
    if (depth > CategoryPolicies.MAX_TREE_DEPTH) {
      throw new BadRequestException(
        `Tree depth cannot exceed ${CategoryPolicies.MAX_TREE_DEPTH} levels`,
      );
    }
  }
  ...
  static validatePathIds(pathIds: string[]): void {
    ...
    if (pathIds.length > CategoryPolicies.MAX_TREE_DEPTH) {
      throw new BadRequestException(
        `Path IDs cannot exceed ${CategoryPolicies.MAX_TREE_DEPTH} items`,
      );
    }
    ...
    for (const id of pathIds) {
      if (typeof id !== 'string' || !cuidPattern.test(id)) {
        throw new BadRequestException(
          `Invalid category ID format in path: ${id}`,
        );
      }
    }
    ...
    if (uniqueIds.size !== pathIds.length) {
      throw new BadRequestException(
        'Duplicate category IDs in path are not allowed',
      );
    }
  }
  ...
  static validateAttributeSlug(slug: string): string {
    ...
    if (normalizedSlug.length > 50) {
      throw new BadRequestException(
        'Attribute slug must not exceed 50 characters',
      );
    }
    ...
    if (!validSlugPattern.test(normalizedSlug)) {
      throw new BadRequestException(
        'Attribute slug must contain only lowercase letters, numbers, and underscores',
      );
    }
    ...
    if (/^[0-9_]/.test(normalizedSlug)) {
      throw new BadRequestException(
        'Attribute slug cannot start with a number or underscore',
      );
    }
  ...
  static validateAllowedValues(
    ...
    if (allowedValues.length === 0) {
      throw new BadRequestException(
        'At least one allowed value is required for select attributes',
      );
    }
    ...
    for (const value of allowedValues) {
      ...
      if (value.length > 100) {
        throw new BadRequestException(
          'Each allowed value must not exceed 100 characters',
        );
      }
    }
  }
  ...
  static validateMoveOperation(
    ...
    if (newDepth > CategoryPolicies.MAX_TREE_DEPTH) {
      throw new BadRequestException(
        `Move would exceed maximum tree depth of ${CategoryPolicies.MAX_TREE_DEPTH}`,
      );
    }
  }

  // Bulk operation validation
  static validateBulkOperationSize(
    itemCount: number,
    ...
    if (itemCount > maxBulkSize) {
      throw new BadRequestException(
        `Bulk ${operationType} cannot exceed ${maxBulkSize} items`,
      );
    }

    if (itemCount === 0) {
      throw new BadRequestException(
        `Bulk ${operationType} requires at least one item`,
      );
    }
  }
}
