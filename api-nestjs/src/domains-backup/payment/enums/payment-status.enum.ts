...
export const PaymentIntentStatusTransitions: Record<PaymentIntentStatus, PaymentIntentStatus[]> = {
...
export const PaymentTransactionStatusTransitions: Record<PaymentTransactionStatus, PaymentTransactionStatus[]> = {
...
export function getValidPaymentIntentTransitions(status: PaymentIntentStatus): PaymentIntentStatus[] {
  return PaymentIntentStatusTransitions[status];
}

export function getValidPaymentTransactionTransitions(status: PaymentTransactionStatus): PaymentTransactionStatus[] {
...
export function isPaymentIntentTerminal(status: PaymentIntentStatus): boolean {
  return [PaymentIntentStatus.SUCCEEDED, PaymentIntentStatus.CANCELED].includes(status);
}

export function isPaymentTransactionTerminal(status: PaymentTransactionStatus): boolean {
  return [
    PaymentTransactionStatus.SUCCEEDED,
    PaymentTransactionStatus.CANCELED,
    PaymentTransactionStatus.DISPUTED,
...
export function isPaymentSuccessful(status: PaymentIntentStatus | PaymentTransactionStatus): boolean {
  return status === PaymentIntentStatus.SUCCEEDED || status === PaymentTransactionStatus.SUCCEEDED;
}
