...
export class RazorpayGateway implements IPaymentGateway {
  ...
  async retrievePaymentIntent(intentId: string): Promise<GatewayResponse<any>> {
    ...
    try {
      ...
      this.logger.error('Failed to retrieve Razorpay order', error, { intentId });
  ...
  async capturePayment(intentId: string, amount?: number): Promise<GatewayResponse<any>> {
    ...
    try {
      ...
      this.logger.error('Failed to capture Razorpay payment', error, { intentId, amount });
  ...
  async createRefund(request: CreateRefundRequest): Promise<GatewayResponse<CreateRefundResponse>> {
    ...
    try {
      ...
      const refund = await this.razorpay!.payments.refund(request.transactionId, refundData);
  ...
  async retrieveRefund(refundId: string): Promise<GatewayResponse<any>> {
    ...
    try {
      ...
      this.logger.error('Failed to retrieve Razorpay refund', error, { refundId });
  ...
  async getSettlement(settlementId: string): Promise<GatewayResponse<any>> {
    ...
    try {
      ...
      this.logger.error('Failed to fetch Razorpay settlement', error, { settlementId });
