...
export interface ICategoryRepository {
  ...
  findAll(filters?: CategoryFilters, pagination?: PaginationOptions): Promise<{ data: Category[]; total: number }>;
  findRoots(filters?: CategoryFilters, pagination?: PaginationOptions): Promise<{ data: Category[]; total: number }>;
  findChildren(parentId: string, filters?: CategoryFilters, pagination?: PaginationOptions): Promise<{ data: Category[]; total: number }>;
  
  // Tree traversal operations
  findAncestors(categoryId: string, includeRoot?: boolean): Promise<Category[]>;
  findDescendants(categoryId: string, maxDepth?: number, activeOnly?: boolean): Promise<Category[]>;
  findSiblings(categoryId: string, filters?: CategoryFilters): Promise<Category[]>;
  findSubtree(rootId: string, maxDepth?: number, activeOnly?: boolean): Promise<Category[]>;
  
  // Path-based operations
  findByPathPrefix(pathPrefix: string, filters?: CategoryFilters): Promise<Category[]>;
  findCategoriesInPaths(paths: string[]): Promise<Category[]>;
  
  // Create and update operations
  create(data: Partial<Category> & { createdBy: string }): Promise<Category>;
  update(id: string, data: Partial<Category> & { updatedBy: string }): Promise<Category>;
  updateStatus(id: string, status: CategoryStatus, updatedBy: string, reason?: string): Promise<Category>;
  
  // Tree modification operations
  move(categoryId: string, newParentId: string | null, movedBy: string, reason?: string): Promise<MoveResult>;
  reorderChildren(parentId: string | null, childrenOrder: { id: string; displayOrder: number }[], updatedBy: string): Promise<void>;
  ...
  // Bulk operations
  bulkCreate(categories: Array<Partial<Category> & { createdBy: string }>): Promise<Category[]>;
  bulkUpdate(updates: Array<{ id: string; data: Partial<Category>; updatedBy: string }>): Promise<Category[]>;
  ...
  validateMove(categoryId: string, newParentId: string | null): Promise<{ isValid: boolean; errors: string[] }>;
  ...
  validateBrandInCategory(brandId: string, categoryId: string): Promise<boolean>;
