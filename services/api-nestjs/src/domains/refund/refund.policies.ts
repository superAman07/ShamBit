...
export class RefundPolicies {
  ...
  static canCreateRefund(
    ...
    switch (userRole) {
      ...
      case 'MERCHANT':
        ...
        return order?.items?.some((item: any) => item.sellerId === userId) || false;
  ...
  static shouldRequireApproval(
    ...
    if (refundReason === RefundReason.FRAUDULENT_TRANSACTION || 
    ...
    return shouldRequireApproval(refundAmount, refundReason, refundType, policy?.autoApprovalLimit);
  }
  ...
  static shouldRestockInventory(
    ...
    return refund.refundType === RefundType.ITEM_LEVEL || refund.refundType === RefundType.FULL;
  }
  ...
  static isRefundEligible(
    ...
    const hasSuccessfulPayment = order.payments?.some((p: any) => p.status === 'COMPLETED');
  ...
  static getRefundFees(
    ...
    if (refundReason === RefundReason.MERCHANT_ERROR || 
    ...
    const refundFee = Math.round((refundAmount * refundFeePercent) / 100) + refundFeeFixed;
  ...
  static detectRefundFraud(
    ...
    const recentRefunds = customerRefundHistory.filter(r => {
      const daysDiff = (new Date().getTime() - r.createdAt.getTime()) / (1000 * 60 * 60 * 24);
      return daysDiff <= 30;
    });
    ...
    if (totalRefundAmount > 50000000) { // ₹5,00,000
      fraudScore += 25;
