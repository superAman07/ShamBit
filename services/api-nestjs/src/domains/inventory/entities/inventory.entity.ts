...
export class Inventory {
  ...
  setThresholds(lowStock?: number, outOfStock?: number): void {
    ...
    if (lowStock !== undefined && outOfStock !== undefined && lowStock < outOfStock) {
      throw new Error('Low stock threshold cannot be less than out of stock threshold');
    }
  ...
  hasReservation(reservationKey: string): boolean {
    return this.reservations?.some(r => r.reservationKey === reservationKey && r.isActive()) || false;
  }
  ...
  getLastMovement(): InventoryLedger | undefined {
    return this.ledgerEntries?.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())[0];
  }
  ...
  validateConsistency(): { isValid: boolean; errors: string[] } {
    ...
    if (calculatedReserved !== this.reservedQuantity) {
      errors.push(`Reserved quantity mismatch: expected ${calculatedReserved}, got ${this.reservedQuantity}`);
    }
    ...
    if (ledgerBalance !== this.totalQuantity) {
      errors.push(`Ledger balance mismatch: expected ${this.totalQuantity}, got ${ledgerBalance}`);
    }
    ...
    if (this.lowStockThreshold !== undefined && 
      ...
      errors.push('Low stock threshold cannot be less than out of stock threshold');
    }
