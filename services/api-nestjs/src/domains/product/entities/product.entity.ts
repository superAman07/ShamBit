...
export class Product {
  ...
  canModerationTransitionTo(newModerationStatus: ProductModerationStatus): boolean {
    const { canModerationTransitionTo } = require('../enums/product-moderation-status.enum');
    return canModerationTransitionTo(this.moderationStatus, newModerationStatus);
  }
  ...
  getEffectiveSeoDescription(): string {
    return this.seoDescription || this.shortDescription || this.description || '';
  }
  ...
  validate(): { isValid: boolean; errors: string[] } {
    ...
    if (this.hasVariants && this.variantAttributes.length === 0) {
      errors.push('Products with variants must have at least one variant attribute');
    }
