...
export const AttributeStatusDescriptions: Record<AttributeStatus, string> = {
  [AttributeStatus.DRAFT]: 'Attribute is being created or edited and not yet available for use',
  [AttributeStatus.ACTIVE]: 'Attribute is active and available for use in products',
  [AttributeStatus.DEPRECATED]: 'Attribute is deprecated but still usable for existing products',
  [AttributeStatus.ARCHIVED]: 'Attribute is archived and no longer usable for new products',
};
...
export const ALLOWED_STATUS_TRANSITIONS: Record<AttributeStatus, AttributeStatus[]> = {
...
export const canTransitionTo = (from: AttributeStatus, to: AttributeStatus): boolean => {
  return ALLOWED_STATUS_TRANSITIONS[from]?.includes(to) || false;
};

export const getAvailableTransitions = (currentStatus: AttributeStatus): AttributeStatus[] => {
...
export const isEditableStatus = (status: AttributeStatus): boolean => {
  return [AttributeStatus.DRAFT, AttributeStatus.ACTIVE, AttributeStatus.DEPRECATED].includes(status);
};